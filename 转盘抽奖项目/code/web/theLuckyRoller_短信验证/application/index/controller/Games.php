...
class Games extends Controller
{
     ...
     * @param $share
     */
    public function beforeTheLuckyRoller($gameId, $share)
    {
        // 获取code
        $this->wx->getWxCode($gameId, $share);
    }
    ...
    public function afterGetWxCode($gameId)
    {
        ...
        if(empty($code) && $this->times < 3){  // 更换浏览器重新获取code
            $this->beforeTheLuckyRoller($gameId, $share);
            ...
            // 有则update  没有则插入数据
            $resId = $this->users->getOneUserInfo(array("openId" => $openid, "aid"=>$gameId), "id");
            if ($resId['id']) {
                // update 用户信息
                $action['uid'] = $resId['id'];
                $this->users->updateIntoUserInfo($userInfo, array("id" => $resId['id'], "aid"=>$gameId));
            } else {
                // insert 用户信息
                $userInfo['aid'] = $gameId;
            ...
            // 获取实物奖励名单
            $goodStuff = $this->goods->getGoodStuff($gameId);
            if(count($goodStuff)>0){
                foreach ($goodStuff as $key => $value){
                    array_unshift($newWining, $value);
                }
            }
        ...
        if (empty($data["realname"]) || strlen($data["realname"]) > 50 ){
            ...
            if(!$code){
                ...
                if ($this->checkCode($data["tel"], $code)){
                    ...
                    $resUpdate = $this->users->updateIntoUserInfo($data, $where);
                    if ($resUpdate["code"] == 60001){
