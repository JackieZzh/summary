...
class vote_activeModuleSite extends WeModuleSite
{
    private $query;
    private $hospital = 'vote_active_hospital';
    private $active = 'vote_active_list';
    private $part = 'vote_active_participant';
    ...
    public function doWebGetActiveInfo()
    {
        ...
        if (empty($_GPC['hid'])){
            ...
            $where['is_del'] = 1;
            $where['h_id'] = $_GPC['hid'];
            $activeList = $this->query->from($this->active)->select('id', 'title', 'add_time',  'sort', 'begin', 'end', 'is_show')->where($where)->orderby($field, $order)->page($page, $limit)->getall();
            foreach($activeList as $key => $value){
                $activeList[$key]['add_time'] = date('m-d', $value['add_time']);
                if(time() >= $value['end']){
                    $activeList[$key]['status'] = 1;
                } else if ( time() >= $value['begin'] && time() < $value['end'] ){
                    $activeList[$key]['status'] = 2;
                } else if ( time() < $value['begin'] ){
                    $activeList[$key]['status'] = 3;
                }
             }
            // 设置session
            $_SESSION['vote_choose_hid'] = array( $_W['uid'] => $_GPC['hid']);
    ...
    public function doWebEditActive()
    {
        ...
        if($_GPC['type'] == 1){
            ...
            $type = 2;
            $static = $_GPC['static'];
            $data = $this->query->from($this->active)->select('id', 'title', 'description', 'background_img', 'goods_url','rote_url', 'background_color', 'begin', 'end', 'add_operator', 'add_time', 'is_show', 'sort', "voter_can", "timer_img", "background_color_rote", 'leader_board_num')->where(array("id" => $hid))->get();
            if(!empty($data)){
                $data['begin'] = date('Y-m-d H:i:s', $data['begin']);
    ...
    public function doWebAddOrUpdateActive()
    {
        ...
        if (empty($_GPC['begin'])){
        ...
        } else if (empty($_GPC['end'])){
            $webData['code'] = 2011;
            $webData['errorMsg'] = "请选择结束时间!";
        } else if (empty($_GPC['title'])){
            $webData['code'] = 2003;
            $webData['errorMsg'] = "活动名不能为空";
        } else if (empty($_GPC['background_img'])){
            $webData['code'] = 2005;
            $webData['errorMsg'] = "请上传背景图";
        } else if (empty($_GPC['timer_img'])){
            $webData['code'] = 2006;
            $webData['errorMsg'] = "请上传倒计时背景图";
        } else if (empty($_GPC['goods_url'])){
            $webData['code'] = 2007;
            $webData['errorMsg'] = "请上奖品图";
        } else if (empty($_GPC['rote_url'])){
            $webData['code'] = 2008;
            $webData['errorMsg'] = "请上规则图";
        } else if (empty($_GPC['background_color'])){
            $webData['code'] = 2009;
            $webData['errorMsg'] = "请设置活动背景色";
        } else if (empty($_GPC['background_color_rote'])){
            $webData['code'] = 2010;
            $webData['errorMsg'] = "请设置规则背景色";
        } else {
            if($_GPC['is_show'] == 'on'){
                $data['is_show'] = 1;
            } else {
            ...
            $data['begin'] = strtotime($_GPC['begin']);
            $data['end'] = strtotime($_GPC['end']);
            $data['title'] = $_GPC['title'];
            $data['description'] = $_GPC['description'];
            $data['background_img'] = $_GPC['background_img'];
            $data['timer_img'] = $_GPC['timer_img'];
            $data['goods_url'] = $_GPC['goods_url'];
            $data['rote_url'] = $_GPC['rote_url'];
            $data['sort'] = $_GPC['sort'];
            $data['background_color'] = $_GPC['background_color'];
            $data['voter_can'] = $_GPC['voter_can'];
            $data['background_color_rote'] = $_GPC['background_color_rote'];
            $data['leader_board_num'] = $_GPC['leader_board_num'];
    ...
    public function doWebGetCode()
    {
        ...
        $value = "http://mob.p028.com/article/yydt/p73a1169.html";
        $filename = IA_ROOT .'/attachment/qrcode/test.png';;
        $code = new QRcode;
        ...
        global $_W, $_GPC;
        require IA_ROOT . '/framework/library/qrcode/phpqrcode.php';
        $url = "https://weixin.prykweb.com/weixintest/vote/public/vote/".$_GPC['id'];
        $value = $url;
        $pathId = $_GPC['id'];
        $title = $_GPC['title'];
        $filename = IA_ROOT .'/attachment/qrcode/'.$pathId.'.png';
        ...
        if (is_file($filename)){
            ...
            // ='text-align: center;display: flex;flex-direction: column'><p style='margin-top: 20px'>扫描左边二维码</p><a style='margin-top: 10px' data_url=$codeUrl data_name='活动$pathId' onclick='downloadIamge()' href='javaScript:;'>下载二维码</a> <a href='javaScript:;'>复制活动链接</a> </div></div> ";
            return " <div style='display: flex;justify-content:space-around'><div><img src= $codeUrl></div><div style
